...
class UVToolsProperties(bpy.types.PropertyGroup):
    reference_mesh: bpy.props.PointerProperty(
        name="Reference Mesh Object",
        type=bpy.types.Object,
        poll=lambda self, obj: obj.type == 'MESH'  # Only show mesh objects
    )
...
class UVToolsPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        props = context.scene.uvtools_properties
        layout = self.layout
        layout.label(text="UV Tools Panel")
        layout.operator("uv.align_seams", text="Snap Seams")
        layout.prop_search(
            props, "reference_mesh",    # Property to store selection
            context.scene, "objects",  # Collection to search in
            text="Select Mesh"         # Label text
        )
...
class UVAlignSeams(bpy.types.Operator):
    ...
    def poll(cls, context):
        ...
        reference_mesh = context.scene.uvtools_properties.reference_mesh
        return (context.object is not None and context.object.mode == 'EDIT'
                and len(selected_meshes) == 2
                and reference_mesh is not None
                and reference_mesh in selected_meshes)

    def initialize_meshes(self, context):
        ...
        reference_mesh = context.scene.uvtools_properties.reference_mesh
        other_meshes = [x for x in bpy.context.selected_objects if x.type == "MESH" and x != reference_mesh]

        if len(other_meshes) == 1:
            self.source_obj = reference_mesh
            self.active_obj = other_meshes[0]
    ...
    def invoke(self, context, event):
        ...
        if context.area.type == 'VIEW_3D':
            ...
            self.initialize_meshes(context)
    ...
    def execute(self, context):
        ...
        if inner_uv_coords:
            transferred_inner_uv_coords = active_sequence.transfer_uv_coordinates(inner_uv_coords, source_sequence.parametric_coordinates)

            active_sequence.set_inner_loops_uv_coordinates(transferred_inner_uv_coords)
            active_sequence.pin_inner_loops()
        if outer_uv_coords:
            transferred_outer_uv_coords = active_sequence.transfer_uv_coordinates(outer_uv_coords, source_sequence.parametric_coordinates)
...
def register():
    bpy.utils.register_class(UVToolsProperties)  # Register this FIRST
    bpy.types.Scene.uvtools_properties = bpy.props.PointerProperty(type=UVToolsProperties)  # Then add to scene
...
def unregister():
    ...
    del bpy.types.Scene.uvtools_properties
    bpy.utils.unregister_class(UVToolsProperties)
